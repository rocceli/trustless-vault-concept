...
import { safeRead } from '@/utils/types';
...
export default function IndexPage() {
  ...
  const getLandingData = async () => {
    try {
      const rate = await safeRead(() => swapCoreHelper.getBTCPrice());
      const stats = await safeRead(() => swapCoreHelper.getProtocolStats());
      const stables = await safeRead(() => liquidPoolHelper.getTotalSupply());
              ...
              {stats.map((stat, index) => (
                <div key={index} className="text-center">
                  ...
                  <div className="text-sm md:text-base text-gray-600 dark:text-gray-400">
                    {loading ? <Skeleton className="h-4 w-20 mx-auto" /> : stat.label}
                  </div>
                </div>
              ))}
            </div>
